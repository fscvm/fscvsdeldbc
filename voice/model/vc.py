...
class DiffVC(BaseModule):
    ...
    @torch.no_grad()
    def convert(self, x, x_lengths, c_source, x_ref, x_ref_lengths, c_ref, n_timesteps, 
                use_ot=True):
        x, x_lengths = self.relocate_input([x, x_lengths])
        x_ref, x_ref_lengths = self.relocate_input([x_ref, x_ref_lengths])
        c_ref, c_source = self.relocate_input([c_ref, c_source])
        x_mask = sequence_mask(x_lengths).unsqueeze(1).to(x.dtype)
        x_ref_mask = sequence_mask(x_ref_lengths).unsqueeze(1).to(x_ref.dtype)
        mean = self.encoder(x, x_mask)
        mean_x = self.decoder.compute_diffused_mean(x, x_mask, mean, 1.0)
        ...
        mean_x_new = torch.zeros((b, self.n_feats, max_length_new), dtype=x.dtype, 
                                  device=x.device)
        x_new = torch.zeros((b, self.n_feats, max_length_new), dtype=x.dtype, 
                             device=x.device)

        for i in range(b):
            mean_new[i, :, :x_lengths[i]] = mean[i, :, :x_lengths[i]]
            mean_x_new[i, :, :x_lengths[i]] = mean_x[i, :, :x_lengths[i]]
            x_new[i, :, :x_lengths[i]] = x[i, :, :x_lengths[i]]

        if not use_ot:
            z = mean_x_new.clone().detach()
            z += torch.randn_like(z, device=z.device)
        else:
            x_ref_ = x_new.clone().detach()
            x_ref_mask_ = x_mask_new.clone().detach()
            mean_ref_ = self.encoder(x_ref_, x_ref_mask_)
            z = self.decoder.forward_diffusion(x_new, x_mask_new, mean_new, x_ref_, 
                                               x_ref_mask_, mean_ref_, c_source, 
                                               n_timesteps=200)
            z[:, 60:, :] = 0.95*mean_new[:, 60:, :] + 0.05*z[:, 60:, :]
        y = self.decoder.reverse_diffusion(z, x_mask_new, mean_new, x_ref, 
                                           x_ref_mask, mean_ref, c_ref, 
                                           n_timesteps)
        return y[:, :, :max_length]
