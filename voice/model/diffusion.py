...
class Diffusion(BaseModule):
    ...
    def estimate_x0(self, xt, mask, mean, xt_ref, ref_mask, c, t):
        t_torch = torch.ones(xt.shape[0], dtype=xt.dtype, device=xt.device, 
                             requires_grad=False) * t
        ex0 = self.estimator(xt, mask, mean, xt_ref, ref_mask, c, t_torch)
        alpha_t = self.get_gamma(0, t, p=1.0)
        sigma2_t = 1.0 - self.get_gamma(0, t, p=2.0)
        ex0 = (xt + ex0*sigma2_t - mean*(1.0 - alpha_t)) / alpha_t
        return ex0
    ...
    def diffuse(self, x0, mask, mean, t):
    ...
    @torch.no_grad()
    def forward_diffusion(self, x0, mask, mean, ref, ref_mask, mean_ref, c, n_timesteps=100):
        h = 1.0 / n_timesteps
        xs = x0 * mask
        for i in range(n_timesteps):
            s = i*h
            t = s + h
            xs_ref = [self.compute_diffused_mean(ref, ref_mask, mean_ref, s)]
            xs_ref = torch.stack(xs_ref, 1)
            ex0 = self.estimate_x0(xs, mask, mean, xs_ref, ref_mask, c, s)
            ...
            xs = 2*xs - mean - (alpha_s*(ex0 - mean) + sigma_s*((xs - mean) - alpha_t*(ex0 - mean))/sigma_t)
            xs *= mask
        return xs

    @torch.no_grad()
    def reverse_diffusion(self, z, mask, mean, ref, ref_mask, mean_ref, c, n_timesteps=100):
        h = 1.0 / n_timesteps
        xt = z * mask
        for i in range(n_timesteps):
            t = 1.0 - i*h
            ...
            ex0 = self.estimate_x0(xt, mask, mean, xt_ref, ref_mask, c, t)
            alpha_s = self.get_gamma(0, s, p=1.0)
            alpha_t = self.get_gamma(0, t, p=1.0)
            sigma_s = math.sqrt(1.0 - self.get_gamma(0, s, p=2.0))
            sigma_t = math.sqrt(1.0 - self.get_gamma(0, t, p=2.0))
            xt = mean + alpha_s*(ex0 - mean) + sigma_s*((xt - mean) - alpha_t*(ex0 - mean))/sigma_t
            xt *= mask
        return xt

    @torch.no_grad()
    def forward(self, z, mask, mean, ref, ref_mask, mean_ref, c, n_timesteps, mode='ml'):
        h = 1.0 / n_timesteps
        xt = z * mask
        for i in range(n_timesteps):
            t = 1.0 - i*h
    ...
    def loss_t(self, x0, mask, mean, x_ref, mean_ref, c, t):
        xt, z = self.diffuse(x0, mask, mean, t)
